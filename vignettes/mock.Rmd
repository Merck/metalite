...
In this example, the available datasets are adae and adsl, which are available in the `r2rtf` R package, i.e., `r2rtf::r2rtf_adae` and `r2rtf::r2rtf_adsl`.
...
Besides, there are two observations for these two analysis: (1) weeks 0-12 and (2) weeks 0-24.
...
```{r}
adae <- r2rtf_adae
adsl <- r2rtf_adsl %>% rename(TRTA = TRT01A)
meta <- meta_adam(
  observation = adae,
  population = adsl
)
...
To define an analysis plan, we use `expand_plan()` to add details per one **mockup** analysis.
...
- `observation = c("wk12", "wk24")`: Weeks 0-12 and Weeks 0-24 analyses.
...
So, we can define an analysis plan as below for a total of 2 mockup to generate 8 TLFs.
...
Then, we use incorporate the above defined plan into the metadata by using `metalite::define_plan()`, i.e.,
...
The following code define the keywords `"wk12"` and `"wk24"`, which is an observation keywords for Weeks 0-12 observations and Weeks 0-24 observations, respectively.
...
For the last two keywords, we can define them by `metalite::define_analysis()`.
...
meta$plan <- meta$plan %>%
  mutate(
    mockup = TRUE,
    output_report = paste0("./mockup/mock-", analysis, ".rtf")
  )
meta$plan
```
...
The pending effort is to define `ae_summary` and `ae_specific` function that can generate the mock up tables based on the inputs.
...
r2rtf:::rtf_convert_format(
  input = "mockup/mock-ae_summary.rtf",
  output_dir = "mockup/",
  format = "pdf"
)
```
...
knitr::include_graphics("mockup/mock-ae_summary.pdf")
```

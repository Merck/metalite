...
The goal is to follow the [dependency inversion principle](https://en.wikipedia.org/wiki/Dependency_inversion_principle)
in software design.

The idea mimics the design of `ggplot2::aes()`.

For a typical analysis, we required developer to provide variable names for

- `name`: a reference name as a key to link other components
- subject id (`id`): typically using "USUBJID" in ADaM data
...
- `label`: a nature language to describe the purpose of this `adam_mapping` object.
...
As long as a study team provide proper data and associate `adam_mapping`,
...
Reference: `eval` and expression. <https://adv-r.hadley.nz/evaluation.html?q=eval#evaluating>
...
A real example can be found in `mkmeta:::default_parameter_ae`
